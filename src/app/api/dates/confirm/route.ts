...
export async function POST(request: NextRequest) {
  try {
    ...
    const { dateId, selectedLocation } = body;
    ...
    const date = await dateService.confirmDate(
      dateId,
      decoded.userId,
      selectedLocation
    );
    ...
    console.error('Confirm date error:', error);
    return NextResponse.json(
      { success: false, error: error.message || 'Failed to confirm date' },
      { status: 500 }
    );
  }
}
