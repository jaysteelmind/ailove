...
export async function POST(request: NextRequest) {
  try {
    ...
    const guide = await coachingService.getDatePreparationGuide({
    ...
    console.error('Get date prep error:', error);
    return NextResponse.json(
      { success: false, error: error.message || 'Failed to get date preparation guide' },
      { status: 500 }
    );
  }
}
